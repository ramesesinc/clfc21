...
class PaymentPostingDetailController 
{
    ...
    @Service('EditWaiverLedgerPostingService')
    def service;
    ...
    def addDetail() {
        ...
        def op = Inv.lookupOpener('edit:posting:waiver:detail:create', [handler: handler, payment: entity]);
        if (!op) return null;
        return op;
    }
    ...
    def editDetail() {
        ...
        def op = Inv.lookupOpener('edit:posting:waiver:detail:edit', [entity: selectedItem, handler: handler]);
        if (!op) return null;
        return op;
    }
}
