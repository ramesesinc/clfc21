...
class SMCPostingController 
{
    @Binding
    def binding;
    
    @Service('SMCPostingService')
    def service;
    
    @Script('Template')
    def template
    
    String title = 'SMC Posting Setting';
    ...
    def getHeaderHtml() {
        ...
        return template.render('html/smc_posting_header_html.gtpl', params);
    }
    ...
    def editCondition( params ) {
        ...
        def op = Inv.lookupOpener('smc:posting:condition:edit', xparams);
        if (!op) return null;
        return op;
    }
