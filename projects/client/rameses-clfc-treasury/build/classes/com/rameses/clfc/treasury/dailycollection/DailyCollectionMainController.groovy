...
class DailyCollectionMainController extends CRUDController 
{
    ...
    def optionsModel = [
        getItems: {
            def invs = Inv.lookupOpeners('dailycollection:plugin');
            def props, list = [];
            invs?.each{ o->
                props = o.properties;
                list << [caption: o.caption, reftype: props.reftype, index: props.index];
            }
            list?.sort{ it.index } 
            return list
        },
    ...
    def getOpener() {
        ...
        def op = Inv.lookupOpener('dailycollection:' + selectedOption?.reftype, params);
        if (!op) return null;
        return op;
    }
