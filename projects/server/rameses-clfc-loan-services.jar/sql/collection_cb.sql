...
FROM (
	...
	FROM collection_cb c
	LEFT JOIN (
		SELECT dc.objid, dc.cbsid
		FROM depositslip d
		INNER JOIN depositslip_cbs dc ON d.objid = dc.parentid
		WHERE d.state NOT IN ('CANCELLED', 'DISAPPROVED')
	) dc ON c.objid = dc.cbsid
...
[xgetLookupListForEncashmentByDate]
SELECT c.*, (SELECT SUM(amount) FROM collection_cb_detail WHERE parentid = c.objid) AS amount
FROM (
	SELECT c.objid
	FROM collection_cb c
...
[findGeneratedCBSFromEncashment]
SELECT c.*
FROM encashment e
INNER JOIN encashment_cbs c ON e.objid = c.parentid
WHERE e.txnstate = 'APPROVED'
	AND c.cbsid = $P{objid}
