...
[getListByMode]
select a.*, ac.dtreleased
from (
	select objid
	from loanapp 
	where appmode = $P{mode}
	...
	inner join loan_product_type p on a.producttype_name = p.name
	inner join borrower b on a.borrower_objid = b.objid
order by a.borrower_name, a.appno desc, ac.dtreleased desc
...
[removeBorrowerIndicesWithNoBorrowerid]
DELETE FROM loanapp_search_index 
WHERE appid=$P{appid} and borrowerid IS NULL 
