...
public class XDBCollectionSheet extends AbstractDBMapper  
{
	...
	public int countByDateAndCollectorWithSegregationid(String date, String collectorid, String segregationid) throws Exception {
		...
		try {
			...
			sql += "WHERE g.billdate = $P{date} AND g.collectorid = $P{collectorid} AND s.segregationtypeid = $P{segregationid}";
	...
	public List<Map> getCollectionSheetsByItemWithSearchtextAndSegregationid(Map params, int limit) throws Exception {
		...
		try {
			String sql = "SELECT c.*, g.description AS route, g.cbsno, " +
					"(SELECT COUNT(p.objid) FROM payment p WHERE p.parentid = c.objid) AS noofpayments, " +
					"(SELECT COUNT(v.objid) FROM void_request v WHERE v.csid = c.objid) AS noofvoids " +
					"FROM " + getTableName() + " c " +
					"INNER JOIN collection_group g ON c.itemid = g.objid " +
					"INNER JOIN collection_group_type t ON g.type = t.type " +
					"INNER JOIN collectionsheet_segregation s ON c.objid = s.collectionsheetid " +
					"WHERE g.billdate = $P{date} " +
					"AND g.collectorid = $P{collectorid} " +
					"AND s.segregationtypeid = $P{segregationid} " +
					"AND c.borrower_name LIKE $P{searchtext} " +
					"ORDER BY t.level, g.description, c.seqno ";
