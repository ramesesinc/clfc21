...
public class XSpecialCollectionRequestController 
{
...
//	private AlertDialog dialog;
	...
	XSpecialCollectionRequestController(UIActionBarActivity activity, ProgressDialog progressDialog, String remarks, String objid) {
...
//			app.specialColSvc.start();
		...
		private void updateRequest( Map params ) throws Exception {
			...
			try {
				...
				specialcollectiondb.execSQL(sql );
		...
		/*
		private void xupdateRequest(Map params) throws Exception {
...
//		}
		...
		private Map getParameters() throws Exception {
			...
			params.put("state", "PENDING");
			params.put("remarks", remarks);
			params.put("orgid", profile.get("ORGID"));
			
			Map collector = new HashMap();
			collector.put("objid", profile.getUserId());
			collector.put("name", profile.getFullName());
