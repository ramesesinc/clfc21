...
public class XSpecialCollectionService 
{
...
//	private LoanPostingService postingSvc = new LoanPostingService();
	...
	public XSpecialCollectionService(ApplicationImpl app) {
		this.app = app;
	...
	private void createTask() {
		actionTask = new Task() {
			...
			private void runImpl() throws Exception {
				...
				StringBuilder specialcollectiondbsb = new StringBuilder();
				
				List<Map> list = specialcollectionpendingservicedb.getPendingSpecialCollection( SIZE );
				for (Map map : list) {
					...
					String collectorid = proxy.getString("collector_objid");
					String xdate = proxy.getString("txndate");
					String date = "";
					if (xdate != null && !xdate.equals("")) {
						date = df.format(df.parse(xdate)); 
					} 
					params.put("billingid", systemdb.getBillingid( collectorid, date ));
					...
					if (response != null && !response.isEmpty()) {
						if (response.containsKey("response")) {
							...
							if ("success".equals( str )) {
								...
								String sql = "delete from specialcollection where objid='" + objid + "';";
								specialcollectiondbsb.append( sql );
								
								sql = "update specialcollection set state='FOR_DOWNLOAD' where objid='" + objid + "';";
								appdbsb.append( sql );
...
//									maindb.execute(sql);
				...
				if (!specialcollectiondbsb.toString().trim().equals("")) {
					...
					try {
						...
						specialcollectiondb.execSQL( specialcollectiondbsb.toString() );
				...
				list = specialcollectionpendingservicedb.getPendingSpecialCollection( 1 );
