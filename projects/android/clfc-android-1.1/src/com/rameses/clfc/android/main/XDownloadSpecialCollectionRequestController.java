...
public class XDownloadSpecialCollectionRequestController 
{
//	private UIActivity activity;
	...
	XDownloadSpecialCollectionRequestController(UIActionBarActivity activity, ProgressDialog progressDialog, Map specialcollection) {
	...
	private Handler successhandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			activity.getHandler().post(new Runnable() {
				public void run() {
					((XSpecialCollectionActivity) activity).loadRequests();
				}
			});
			if (progressDialog.isShowing()) progressDialog.dismiss();
...
//				}
		...
		private void saveSpecialCollection( Map data ) throws Exception {
			...
			String billingid = MapProxy.getString(data, "billingid");
			...
			specialcollectiondb.changeStateById(MapProxy.getString(specialcollection, "objid"), "DOWNLOADED");
			
			if (!proxy.isEmpty()) {
				Map params = new HashMap();
				params.put("objid", proxy.getString("objid"));
				params.put("state", "ACTIVE");
				params.put("description", MapProxy.getString(specialcollection, "name"));
			...
			List<Map> billings = (List<Map>) data.get("list");
			...
			for (Map item : billings) {
				...
				if (proxy.containsKey("notes")) {
					...
					for (Map map : list) {
						...
						String txndate = new SimpleDateFormat("yyyy-MM-dd").parse(m.getString("dtcreated")).toString();
		...
		/* old code for save collection
		private void xsaveSpecialCollection(Map map) throws Exception {
