...
//	private LoanPostingService svc = new LoanPostingService();
	...
	public XCaptureService(ApplicationImpl app) {
		this.app = app;
	...
	private void createTask() {
		actionTask = new Task() {
			...
			private void runImpl() throws Exception {
				List<Map> list = capturepaymentdb.getForUploadPayments( SIZE );
				execPayments( list );
				
				boolean haspayment = capturepaymentdb.hasForUploadPayments();
				if (haspayment == false) {
					serviceStarted = false;
					this.cancel();
				}
...
//				
			...
			*/
			private void execPayments( List<Map> list ) throws Exception {
				if (!list.isEmpty()) {
