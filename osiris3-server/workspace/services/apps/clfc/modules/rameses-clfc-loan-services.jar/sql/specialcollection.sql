...
SELECT s.*,
	CASE 
		WHEN f.objid IS NULL THEN 'Special' ELSE 'Follow-up'
	END AS billingtype
FROM specialcollection s
LEFT JOIN followupcollection f ON s.objid = f.objid 
WHERE s.txntype = 'REQUEST'
...
[getSpecialBillingList]
SELECT l.*, r.code AS route_code, r.area AS route_area,
	r.description AS route_description, a.objid AS loanappid,
	a.appno, b.address AS homeaddress, ac.dtreleased AS loandate,
	a.loanamount
FROM specialbillingaccount sb
INNER JOIN loan_ledger l ON sb.ledger_objid = l.objid
