...
[getWithPartialPayments]
SELECT p.*, CASE WHEN cw.objid IS NULL THEN FALSE ELSE TRUE END AS withpartial
FROM (
	SELECT p.objid
	FROM loan_ledger_payment p
	LEFT JOIN loan_ledger_proceeds pr ON p.objid = pr.refid
	WHERE p.isonline = 1
		AND pr.objid IS NULL
		${filter}
	UNION
	SELECT p.objid
	FROM loan_ledger_payment p
	LEFT JOIN loan_ledger_noncash nc ON p.objid = nc.refid
	WHERE  p.isonline = 1
		AND nc.objid IS NULL
		${filter}
) q 
	INNER JOIN loan_ledger_payment p ON q.objid = p.objid
	LEFT JOIN collection_withpartial cw ON p.objid = cw.paymentid
ORDER BY p.txndate DESC, p.refno
